...
function App() {
  ...
  const handleIngest = async (e) => {
    ...
    try {
      ...
      const payload =
        ingestType === "note"
          ? { type: "note", content: textContent }
          : { type: "url", content: urlContent };

      const response = await axios.post(`${API_BASE}/ingest`, payload);

      setSuccess(
        `Content ingested successfully! Created ${response.data.chunks} chunks.`
      );
                  ...
                  {sources.map((source, idx) => (
                    <div key={idx} className="source">
                      <div className="source-header">
                        Source {idx + 1} | Item ID: {source.item_id} | Score:{" "}
